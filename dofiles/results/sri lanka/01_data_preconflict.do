...
forvalues i = 1/`col_years' {
	...
	*************************
	if data[2,`i'] == 0 {
		...
		* Preparing variables
		keep countrycode year hhid pid fexp_s welfare male urban age relationharm educat7 hsize ipcf itf ip inp ila icap ijubi inla_otro itranp itrane renta_imp itranext_m itranext_nm itranint_m itranint_nm itran_ns cpi${ppp} icp${ppp} hogarsec empstat* lstatus* occup* industry* pline_nat2
	...
	****************************
	if data[2,`i'] == 1 {
		...
		qui use "${data_path}/${country}\Data\preconflict/${country}_`year'_6s_dom_yes_int_yes_inc_yes_cons_no_matching_yes_st_yes.dta", clear
