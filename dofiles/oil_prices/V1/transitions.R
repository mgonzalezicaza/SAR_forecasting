...
ggplot(Basetot, aes(y = value_m, axis1 = country, axis2 = condition_post)) + geom_alluvium(aes(fill = condition_pre), width = 1/12) + geom_stratum(width = 1/12, fill = "gray", color = "black") + geom_label(stat = "stratum", aes(label = after_stat(stratum))) + scale_x_discrete(limits = c("Country", "Post-conflict"), expand = c(.05, .05)) + scale_fill_brewer(type = "qual", palette = "Set2") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ labs(y= "Million people", fill="Class baseline") + ggtitle("Class transitions due to Iran's conflict");
